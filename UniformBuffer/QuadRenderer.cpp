...
void QuadRenderer::initResources()
{
	...
	QMatrix4x4 identify;
	// 遍历并发帧计数器（concurrentFrameCount）范围内的所有帧
	for (int i = 0; i < concurrentFrameCount; i++)
	{
		// 计算当前帧的偏移量，基于帧索引乘以每帧的uniform buffer分配大小
		vk::DeviceSize offset = i * uniformAllocSize;
		// 将单位矩阵的constData（即连续的16个float值）复制到uniform缓冲区映射内存的相应偏移处
		memcpy(uniformBufferMemPtr + offset, identify.constData(), 16 * sizeof(float));
		// 更新uniformBufferInfo数组中的当前帧信息
		uniformBufferInfo_[i].buffer = uniformBuffer_;		// 设置缓冲区
		uniformBufferInfo_[i].offset = offset;				// 设置偏移量
		uniformBufferInfo_[i].range = uniformAllocSize;		// 设置范围
	}
	device.unmapMemory(uniformDevMemory_);

	// 创建一个描述符池大小结构，指定类型为VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER，数量为concurrentFrameCount
	vk::DescriptorPoolSize descPoolSize(vk::DescriptorType::eUniformBuffer, (uint32_t)concurrentFrameCount);
	vk::DescriptorPoolCreateInfo descPoolInfo;
	descPoolInfo.maxSets = concurrentFrameCount;			// 最大集合数等于并发帧计数
	descPoolInfo.poolSizeCount = 1;
	descPoolInfo.pPoolSizes = &descPoolSize;				// 指向描述符池大小结构体的指针
	// 使用设备创建描述符池
	...
	// 遍历并发帧计数器范围内的所有帧
	for (int i = 0; i < concurrentFrameCount; ++i) {
		// 创建描述符集分配信息结构体，指定使用当前描述符池、分配1个描述符集、使用descSetLayout_
