...
{
	auto mesh_vert = readFile("./mesh_vert.spv");
	auto mesh_frag = readFile("./mesh_frag.spv");

	vk::ShaderModuleCreateInfo shaderInfo;
	shaderInfo.codeSize = mesh_vert.size();
	shaderInfo.pCode = reinterpret_cast<uint32_t*>(mesh_vert.data());
	vk::ShaderModule vertShader = device_.createShaderModule(shaderInfo);

	shaderInfo.codeSize = mesh_frag.size();;
	shaderInfo.pCode = reinterpret_cast<uint32_t*>(mesh_frag.data());
