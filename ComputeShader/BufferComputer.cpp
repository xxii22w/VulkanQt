...
void BufferComputer::initResources()
{
	...
	bufferInfo.usage = vk::BufferUsageFlagBits::eStorageBuffer;		// 设置缓冲区用途为存储缓冲区（Storage Buffer）
	bufferInfo.sharingMode = vk::SharingMode::eExclusive;			// 设置缓冲区的共享模式为独占模式（Exclusive Mode），即同一时刻只能被一个队列家族访问
	...
	auto image_comp = readFile("./buffer.spv");

	vk::ShaderModuleCreateInfo shaderInfo;
	shaderInfo.codeSize = image_comp.size();
	shaderInfo.pCode = reinterpret_cast<uint32_t*>(image_comp.data());
